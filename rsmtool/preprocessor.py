    ...
    def check_feature_subset_file(cls, df, subset=None, sign=None):
        ...
        if sign:
            ...
            if len(existing_sign_columns) > 1:
                ...
                raise ValueError(
                    f"The feature_subset_file must contain the requested sign column 'sign_{sign}'"
                )
            else:
...
class FeatureSpecsProcessor:
    ...
    @classmethod
    def generate_specs(
        cls, df, feature_names, train_label, feature_subset=None, feature_sign=None
    ):
        """
        ...
        for feature in feature_names:
            ...
            feature_dict["sign"] = FeatureSpecsProcessor.find_feature_sign(
                feature, sign_dict
            )
...
class FeaturePreprocessor:
    ...
    def check_model_name(model_name):
        ...
        if is_built_in_model(model_name):
            ...
            raise ValueError(
                f"The specified model {model_name} was not found. Please check the spelling."
            )

        return model_type

    @staticmethod
    def trim(values, trim_min, trim_max, tolerance=0.4998):
        """
    ...
    @staticmethod
    def remove_outliers_using_truncations(values, feature_name, truncations):
        """
    ...
    @staticmethod
    def select_candidates(df, N, candidate_col="candidate"):
        """
    ...
    def filter_on_column(
        df, column, id_column, exclude_zeros=False, exclude_zero_sd=False
    ):  # noqa: D301
        """
    ...
    def process_predictions(
        ...
        df_pred_process["raw_trim"] = FeaturePreprocessor.trim(
        ...
        df_pred_process["scale_trim"] = FeaturePreprocessor.trim(
    ...
    def filter_data(
        ...
        if min_candidate_items:
            (
                ...
                df_excluded_candidates,
            ) = FeaturePreprocessor.select_candidates(df_filtered, min_candidate_items)
    ...
    def process_data_rsmtool(self, config_obj, data_container_obj):
        ...
        if isinstance(feature_field, list):
            ...
            feature_specs = FeatureSpecsProcessor.validate_feature_specs(
        ...
        if generate_feature_specs_automatically:
            if select_transformations:
                ...
                feature_specs = FeatureSpecsProcessor.generate_specs(
                    df_train_features,
                    feature_names,
                    "sc1",
                    feature_subset=feature_subset,
                ...
                feature_specs = FeatureSpecsProcessor.generate_default_specs(
                    feature_names
                )
