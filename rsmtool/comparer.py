    ...
    def compute_correlations_between_versions(df_old,
        ...
        for feature in features:
            ...
            df_cor = pd.DataFrame({'Feature': [feature],
                                   'N': len(df_merged),
                                   'human_old': pearsonr(df_merged[f'{human_score}%%%old'],
                                                         df_merged[f'{feature}%%%old'])[0],
                                   'human_new': pearsonr(df_merged[f'{human_score}%%%new'],
                                                         df_merged[f'{feature}%%%new'])[0],
                                   'old_new': pearsonr(df_merged[f'{feature}%%%new'],
                                                       df_merged[f'{feature}%%%old'])[0]})
    ...
    def load_rsmtool_output(self, filedir, figdir, experiment_id, prefix, groups_eval):
        ...
        for group in groups_eval:
            ...
            if exists(group_eval_file):
                ...
                files[f'df_eval_by_{group}_m_sd'] = df_eval[['N',
                                                             'H1 mean',
                                                             'H1 SD',
                                                             'score mean(br)',
                                                             'score SD(br)',
        ...
        for group in groups_eval:
            ...
            if exists(group_pcor_file):
                files[f'df_pcor_sc1_by_{group}'] = DataReader.read_from_file(group_pcor_file,
                                                                             index_col=0)
        ...
        for group in groups_eval:
            group_mcor_file = join(filedir,
                                   f'{experiment_id}_margcor_score_by_{group}.{file_format}')
            if exists(group_mcor_file):
                files[f'df_mcor_sc1_by_{group}'] = DataReader.read_from_file(group_mcor_file,
                                                                             index_col=0)
        ...
        if exists(pca_file):
            ...
            files['df_pcavar'] = DataReader.read_from_file(join(filedir,
                                                                f'{experiment_id}_pcavar.{file_format}'),
                                                           index_col=0)
        ...
        if exists(descriptives_file):
            ...
            # join with df_features_n_values to get the value of N
            percentiles_file = join(filedir, f'{experiment_id}_feature_descriptivesExtra.{file_format}')
        ...
        for group in groups_eval:
            feature_boxplot_prefix = join(figdir,
                                          f'{experiment_id}_feature_boxplot_by_{group}')
