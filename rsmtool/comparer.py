...
class Comparer:
    ...
    def load_rsmtool_output(self, filedir, figdir, experiment_id, prefix, groups_eval):
        ...
        dis_corr_file = join(
            filedir,
            f"{experiment_id}_disattenuated_correlations.{file_format}",
        )
        ...
        for group in groups_eval:
            group_pcor_file = join(
                filedir,
                f"{experiment_id}_pcor_score_by_{group}.{file_format}",
            )
        ...
        if exists(pca_file):
            ...
            files["df_pcavar"] = DataReader.read_from_file(
                join(filedir, f"{experiment_id}_pcavar.{file_format}"),
                index_col=0,
            )
        ...
        if exists(descriptives_file):
            ...
            percentiles_file = join(
                filedir,
                f"{experiment_id}_feature_descriptivesExtra.{file_format}",
            )
